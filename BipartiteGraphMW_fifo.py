...
def run_mw_fifo_simulation(inputRates, processRates, T, learning_rate, numQueues, numServers,
    ...
    """Optimized FIFO MW simulation with JIT compilation"""
    ...
    for t in range(T):
        ...
        for q in range(numQueues):
            for j in range(accessible_lengths[q]):
                cost = costs[q, j]
                weights[q, j] *= 1 - learning_rate * cost
...
###WITH BUFFERS###
##################
print("Starting optimized FIFO MW simulation...")
...
for m in range(M):
    ...
    for r in range(sample):
        ...
        sumBuildup = run_mw_fifo_simulation(
            inputRates, processRates, T, learning_rate, numQueues, numServers,
            arrivals, weight_randoms, processed,
            accessible_matrix, accessible_lengths
        )
...
# Generate Figure showing buildup in MW system
