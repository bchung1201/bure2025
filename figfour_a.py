...
arrival_rates = [1/3,1/6]

service_rates = [2/3,2/9,1/9]
...
for i in range(len(arrival_rates)):
    for j in range(len(service_rates)):
        if j == 0:
            plt.plot(x_axis, (1-gamma)*np.array(weights[i][j]) + gamma/m, label = ("Q" + str(i+1) +  " strategy") , linestyle=linestyles[0], color=colors[i])
        else:
            plt.plot(x_axis, (1-gamma)*np.array(weights[i][j]) + gamma/m, linestyle=linestyles[0], color=colors[i])
